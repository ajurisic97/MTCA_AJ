...
public class LocalCacheService : ICacheService
{
    ...
    public void SetAbs<T>(string key, T value, DateTimeOffset? absoluteExpiration = null)
    {
        _cache.Set(key, value, new MemoryCacheEntryOptions { AbsoluteExpiration = absoluteExpiration });
        _logger.LogDebug($"Added to Cache : {key}", key);
    }

    public Task SetAbsAsync<T>(string key, T value, DateTimeOffset? absoluteExpiration = null, CancellationToken token = default)
    {
        SetAbs(key, value, absoluteExpiration);
        return Task.CompletedTask;
    }
